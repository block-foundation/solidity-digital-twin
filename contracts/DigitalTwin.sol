...
/**
 *  @title DigitalTwin
 *  @notice This contract is used for creating a digital twin of a building 
...
contract DigitalTwin is ChainlinkClient {
    ...
    /**
     *  @notice An array containing all the maintenance reports for 
     *  the building
     ...
     *  @dev Can only be called by the contract owner
     */
    function addMaintenanceReport(
        string memory description
    ) public onlyOwner {

        maintenanceReports.push(
            MaintenanceReport(description, block.timestamp)
        );
     ...
     *  @dev Can only be called by the contract owner
     */
    function updateHumidity() public onlyOwner returns (
        bytes32 requestId
    ) {
        Chainlink.Request memory request = buildChainlinkRequest(
            jobId, address(this),
    ...
    function updateOccupancy() public onlyOwner returns (bytes32 requestId) {
        Chainlink.Request memory request = buildChainlinkRequest(
            jobId,
            address(this),
            this.fulfillOccupancy.selector
        );
        return sendChainlinkRequestTo(oracle, request, fee);
    }
     ...
     *  @dev Can only be called by the contract owner
     */
    function updateEnergyConsumption() public onlyOwner returns (
        bytes32 requestId
    ) {
        Chainlink.Request memory request = buildChainlinkRequest(
            jobId, address(this),
            this.fulfillEnergyConsumption.selector
        );
        return sendChainlinkRequestTo(oracle, request, fee);
    }
     ...
     *  @dev Can only be called by the Chainlink oracle
     */
    function fulfillEnergyConsumption(
        bytes32 _requestId,
     ...
     *  @return requestId The requestId of the Chainlink request
     */
    function updateStructuralHealth() public onlyOwner returns (
        bytes32 requestId
    ) {
        Chainlink.Request memory request = buildChainlinkRequest(
            jobId,
            address(this),
            this.fulfillStructuralHealth.selector
        );
        return sendChainlinkRequestTo(oracle, request, fee);
    }
     ...
     *  @dev Can only be called by the Chainlink oracle
     */
     function fulfillStructuralHealth(
        bytes32 _requestId,
        uint256 _structuralHealth
    ) public recordChainlinkFulfillment(_requestId) {
